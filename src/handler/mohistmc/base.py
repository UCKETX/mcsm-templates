    ...
    async def load_builds(self) -> None:
        ...
        for build_info in self.builds_list:
            try:
                ...
                SyncLogger.warning(
                    "{project_name} | {version} | Failed to create build from info: {build_info}, error: {e}".format(
                        project_name=self.project_name, version=self.version, build_info=build_info, e=e
                    )
                )
                continue
